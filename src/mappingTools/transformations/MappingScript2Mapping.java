...
public class MappingScript2Mapping{
  public String script;
  public ArrayList<String> primitives;
  public MappingScript2Mapping(String txt ){
script = txt.replaceAll("\n", " ");
script = script.substring(script.indexOf("MappingScript{") + 13+1,script.indexOf("}"));
...
for (Method m : MappingEditor.class.getMethods()){
primitives.add(m.getName());
}

  }
  public Mapping transformation(){
    Mapping mapping= MappingFactory.eINSTANCE.createMapping();
    mappingTools.editor.MappingEditor mappingEditor = new mappingTools.editor.MappingEditor(mapping);
	 parseExpressions(mappingEditor);
	return mapping;

  }
  public void transformation(Mapping mapping ){
    mappingTools.editor.MappingEditor mappingEditor = new mappingTools.editor.MappingEditor(mapping);
	 parseExpressions(mappingEditor);

  }
  public void parseExpressions(mappingTools.editor.MappingEditor mappingEditor ){
...
triggerMethod(mappingEditor, parseExpressionArguments(expr) ,prim);
}
}
}
