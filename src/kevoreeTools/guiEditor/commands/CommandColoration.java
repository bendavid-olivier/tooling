...
public class CommandColoration extends Command implements Action{
  public CommandColoration(KevoreeTextualEditor e ,String nme ,String desc ){
		super(e, nme, desc);
  ...
  public void colorationPolicyScript(String text ,final StyledDocument doc ){
		Pattern p = Pattern.compile("(KevoreeScript)");
		Matcher m = p.matcher(text);
		while (m.find() == true) {
			MutableAttributeSet attri = new SimpleAttributeSet();
